---
name: init
description: Set up docsync in the current case repo and produce the first full documentation pass.
disable-model-invocation: true
argument-hint: "--space <KEY> --root-page <url-or-id> [--title-prefix '[X] '] [--case-name <name>] [--publish-mode ci|local]"
allowed-tools: Bash Read Edit Write Grep Glob Skill
---

You are onboarding a new client case. Output: a configured `.docsync/`, a filled-in case brief, nine populated
pages, a CI workflow, and (if credentials exist) a first publish.

1. **Inputs.** Parse `$ARGUMENTS` for `--space`, `--root-page`, `--title-prefix`, `--case-name`, `--publish-mode`,
   `--docs-dir`. If the repo already has a docsync case (`python3 .docsync/bin/docsync.py status`), this init ADDS a
   project: `--case-name` is required, the pages default to `docs/<case>/`, and the existing case is moved to
   `.docsync/cases/<case>/` automatically (commit those moves in the same PR). Shared repos should scope
...
2. **Bootstrap.** `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/docsync.py init <args>`. First case in a repo: writes
   `.docsync/config.yaml`, `.docsync/case-context.md`, `.docsync/audit.jsonl`, `.docsync/bin/` (vendored tool),
   `docs/confluence/*.md` templates and `.github/workflows/docsync.yml`. Additional case: writes
   `.docsync/cases/<case>/{config.yaml,case-context.md,audit.jsonl}`, `docs/<case>/*.md`, keeps shared settings
   (trunk, publish mode) in `.docsync/config.yaml`, and adds the docs dir to the workflow's publish `paths:`.
   Every later step below uses that case's files; pass `--case <case>` to docsync commands when there are several. Review `watch_paths`/`ignore_paths` in the config against the
   repo layout and adjust.
3. **Case brief.** Fill `.docsync/case-context.md` from everything available: README, CLAUDE.md, diagrams,
