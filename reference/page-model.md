...
| Engineering conventions (`70-conventions.md`) | Naming, layout, logging/observability/tracing, DQ tests, environments, local runs | Case-specific business rules |
| Glossary (`80-glossary.md`) | One line per term | Multi-paragraph explanations |
| Change Log (generated) | Rendered from `<case_dir>/audit.jsonl`; never edited by hand | — |
...
## Adding a page
Only when an entity class does not fit anywhere above (for example a client-facing data contract).
